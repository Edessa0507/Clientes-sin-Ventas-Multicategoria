...
const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 px-3 py-3 sm:px-6 lg:px-8">
      {/* Botón de tema */}
      <button
        onClick={toggleDarkMode}
        className="fixed top-3 right-3 p-3 rounded-xl bg-white dark:bg-gray-800 shadow-lg hover:shadow-xl transition-all duration-200 z-10 touch-manipulation"
      >
        {darkMode ? (
      ...
      <motion.div
        ...
        className="max-w-sm w-full space-y-5 sm:max-w-md sm:space-y-6"
      >
        {/* Header */}
        <div className="text-center">
          <motion.div
            ...
            className={`mx-auto h-18 w-18 sm:h-16 sm:w-16 rounded-full flex items-center justify-center mb-5 ${
              loginType === 'vendedor' 
          ...
          >
            <IconComponent className={`h-9 w-9 sm:h-8 sm:w-8 ${
              loginType === 'vendedor' 
          ...
          <p className="mt-3 text-base sm:text-sm text-gray-600 dark:text-gray-400 px-2">
            {currentConfig.description}
          </p>
        </div>

        {/* Selector de tipo de usuario */}
        <div className="flex space-x-1 bg-gray-200 dark:bg-gray-700 p-1.5 rounded-xl">
          {Object.entries(loginTypeConfig).map(([type, config]) => {
            ...
            return (
              <button
                ...
                className={`flex-1 flex items-center justify-center space-x-2 py-3 px-2 sm:px-3 rounded-lg text-sm sm:text-sm font-medium transition-all duration-200 touch-manipulation ${
                  loginType === type
                    ? type === 'vendedor' 
          ...
          <div className="card p-6">
            <div className="space-y-5">
              {loginType === 'admin' ? (
                ...
                <>
                  <div>
                    <label htmlFor="email" className="block text-base sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                      Correo electrónico
                    </label>
                    <input
                      id="email"
                      ...
                      className="input-field text-base h-12 sm:h-10 sm:text-sm"
                      placeholder="admin@empresa.com"
                    />
                  </div>
                  
                  <div>
                    <label htmlFor="password" className="block text-base sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                      Contraseña
                    </label>
                    <div className="relative">
                      <input
                        ...
                        onChange={handleInputChange}
                        className="input-field text-base h-12 sm:h-10 sm:text-sm pr-12"
                        placeholder="••••••••"
                      />
                      <button
                        type="button"
                      ...
                      >
                        {showPassword ? (
                          <EyeOff className="h-6 w-6 sm:h-5 sm:w-5 text-gray-400" />
                        ) : (
                          <Eye className="h-6 w-6 sm:h-5 sm:w-5 text-gray-400" />
                        )}
                      </button>
                    </div>
                  </div>
                ...
                <>
                  <div>
                    <label htmlFor="codigo" className="block text-base sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                  ...
                  {formData.codigo && (
                    <motion.div
                      ...
                      className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 border-l-4 border-primary-500"
                    >
                      <div className="flex items-center space-x-3">
                        <div className="flex-1">
                          {nameLoading ? (
                            <div className="flex items-center space-x-3">
                              <div className="w-5 h-5 animate-spin rounded-full border-2 border-gray-300 border-t-primary-600"></div>
                            ...
                            <p className="text-base sm:text-sm text-red-600 dark:text-red-400">
            ...
            <div className="mt-6">
              <button
                ...
                className={`w-full flex justify-center py-4 px-4 border border-transparent rounded-xl shadow-sm text-base sm:text-sm font-medium text-white transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-offset-2 touch-manipulation ${
                  loginType === 'vendedor' 
              ...
              >
                {loading ? (
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-base sm:text-sm">Iniciando sesión...</span>
                  </div>
                ) : (
                  <span className="text-base sm:text-sm">Iniciar sesión como {currentConfig.title}</span>
                )}
              </button>
            </div>
          </div>
